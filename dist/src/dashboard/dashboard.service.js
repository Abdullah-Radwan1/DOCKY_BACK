...
let DashboardService = class DashboardService {
  ...
  async getSummary(userId) {
    const now = new Date();
    const in30Days = new Date(now.getTime() + 90 * 24 * 60 * 60 * 1000);
    const [
      docStatusGroups,
    ...
    const complianceDistribution =
      this.buildComplianceDistribution(verdictGroups);
    const riskDistribution = this.buildRiskDistribution(riskGroups);
    const findingsSeverityBreakdown =
      this.buildSeverityBreakdown(severityGroups);
    const documentStatusBreakdown =
      this.buildDocStatusBreakdown(docStatusGroups);
    ...
    const documentsRequiringAttention = await Promise.all(
