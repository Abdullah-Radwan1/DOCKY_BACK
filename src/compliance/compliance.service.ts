...
export class ComplianceService {
  ...
  async submitAnalysis(
    dto: CreateAnalysisRequestDto & { ip?: string },
  ): Promise<{ requestId: string }> {
    const methodStart = Date.now();
    this.logger.log(
      `[FLOW START] submitAnalysis for documentId: ${dto.documentId}, userId: ${dto.userId}`,
    );
    ...
    this.logger.log(
      `[AWAIT END] policyService.enforceAnalysisLimit took ${Date.now() - limitStart}ms`,
    );

    // ── Validate document exists and is ready ────────────────────────────
    this.logger.log(
      `[AWAIT START] prisma.document.findUnique for id: ${dto.documentId}`,
    );
    ...
    this.logger.log(
      `[AWAIT END] prisma.document.findUnique took ${Date.now() - docQueryStart}ms`,
    );
    ...
    if (dto.userId) {
      if (document.uploadedBy !== dto.userId) {
        this.logger.warn(
          `[FLOW ERROR] Document owner mismatch for user ${dto.userId}`,
        );
        throw new NotFoundException('Document not found');
      }
    } else {
      if (document.guestToken !== guestIp) {
        this.logger.warn(
          `[FLOW ERROR] Document guest token mismatch for IP ${guestIp}`,
        );
        throw new NotFoundException('Document not found');
      }
    }
    ...
    if (document.status !== 'ready') {
      this.logger.warn(
        `[FLOW ERROR] Document ${dto.documentId} not ready (status: ${document.status})`,
      );
    ...
    this.logger.log(
      `[AWAIT END] policyService.incrementAnalysis took ${Date.now() - incStart}ms`,
    );

    // ── Fire the AI pipeline in background (non-blocking) ────────────────
    this.logger.log(
      `[FLOW BACKGROUND TRIGGER] Firing orchestrator.analyzeDocument for requestId: ${request.id}`,
    );
    ...
    void this.orchestrator
      ...
      .then(async () => {
        ...
        if (!dto.userId) {
          this.logger.log(
            `[FLOW BACKGROUND INFO] Skipped notifications (no userId)`,
          );
          return;
        }
        try {
          ...
          this.logger.log(
            `[AWAIT END] getAnalysisResult for notifications took ${Date.now() - notifFetchStart}ms`,
          );
          ...
          if (result.response?.AnalysisResult) {
            ...
            const docName =
              result.document?.originalFileName ?? 'your document';
            const isHighRisk = risk === 'high' || verdict === 'non_compliant';

            this.logger.log(
              `[AWAIT START] notificationsService.createNotification`,
            );
            const notifCreateStart = Date.now();
            await this.notificationsService.createNotification({
              userId: dto.userId,
              title: isHighRisk
                ? '⚠️ Risk Alert Detected'
                : 'Analysis Complete',
              message: isHighRisk
            ...
            this.logger.log(
              `[AWAIT END] notificationsService.createNotification took ${Date.now() - notifCreateStart}ms`,
            );
          } else {
            this.logger.warn(
              `[FLOW BACKGROUND WARN] No AnalysisResult found to trigger notifications`,
            );
          }
        } catch (notifErr) {
          this.logger.error(
    ...
    this.logger.log(
      `[FLOW RETURN] submitAnalysis returning requestId: ${request.id} (total response time: ${Date.now() - methodStart}ms)`,
    );
  ...
  async getAnalysisResult(requestId: string) {
    ...
    if (!result) {
      throw new NotFoundException(`Analysis request ${requestId} not found`);
    }
